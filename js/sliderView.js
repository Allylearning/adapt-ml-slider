...
export default class SliderView extends QuestionView {
  ...
  onHandleFocus(event) {
    ...
    // Me Learning change. Draws the circles in the
  fillCircles(pixels) {
    const notches = this.$('.slider__scale-notch');
    _.each(notches, function($notch) {
        if(pixels >= parseInt($($notch).css('left'), 10)) {
        $($notch).addClass('passed');
        } else {
        $($notch).removeClass('passed');
        }
    });
  }
  ...
  showCorrectAnswer() {
    ...
    if (answers.length === 0) {
      console.log('adapt-ml-slider::WARNING: no correct answer or correct range set in JSON');
      return;
    }
